...
- Methods and attributes that returned :class:`TextChannel` (such as :func:`Client.get_channel`) can now return :class:`Thread`.
- Python 3.8 or newer is required.
- Attributes that returned :class:`Asset` are renamed, eg attributes ending with ``_url`` (eg ``avatar_url``) are changed to :Attr:`avatar.url`. :attr:`User.avatar` returns ``None`` in case the default avatar is used.
...
- Edit methods no longer update the cache and instead return modified instances.
- Selfbots are no longer supported.
- ``Client.logout`` is removed; use :meth:`Client.close` instead.
- ``on_private_channel_create`` and ``on_private_channel_delete`` events are removed.
...
- Many arguments are now specified as positional-only or keyword-only; eg :func:`oauth_url` now takes keyword-only arguments, and methods starting with ``get_`` or ``fetch_`` take positional-only arguments.
...
- Asset-related attributes that previously returned hash strings (eg :attr:`User.avatar`) now returns :class:`Asset`. :attr:`Asset.key` returns the hash from now on.
- ``Class.x_url`` and ``Class.x_url_as`` are removed. :meth:`Asset.replace`, :meth:`Asset.with_size`, :meth:`Asset.with_format`, and :meth:`Asset.with_static_format` methods can be used to get specific asset sizes or types.
...
Note that newly-added :func:`discord.utils.utcnow()` can be used as an alias of ``datetime.datetime.now(datetime.timezone.utc)``.

``Embed.__bool__`` change
^^^^^^^^^^^^^^^^^^^^^^^^^

:class:`Embed` is now always considered truthy if *any* of the content is set.

Duplicate registration of cogs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:meth:`commands.Bot.add_cog` now raises when a cog with the same name is already registered. The ``override`` argument can be used to bring back the 1.x behavior.
...
:attr:`commands.Command.clean_params <ext.commands.Command.clean_params>` is now a :class:`dict`, not :class:`collections.OrderedDict`. The behaviour is the same however - dictionaries are ordered as of 3.7.
...
:exc:`commands.BadInviteArgument` now requires one argument, ``argument``.
...
:exc:`commands.CommandOnCooldown` now requires an additional argument, type.
...
This was moved to :attr:`commands.Context.clean_prefix`.
