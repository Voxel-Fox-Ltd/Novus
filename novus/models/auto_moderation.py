...
class AutoModerationTriggerMetadata:
    ...
    def __init__(
        ...
        *,
        ...
        self.mention_total_limit = (
            mention_total_limit if mention_total_limit is not None else None
        )
    ...
    @classmethod
    def _from_data(
        cls, *, data: AutoModerationTriggerMetadataPayload
    ) -> AutoModerationTriggerMetadata:
        return cls(
...
class AutoModerationRule(AutoModerationAPIMixin):
    ...
    def __init__(self, *, state: HTTPConnection, data: AutoModerationRulePayload):
        self._state = state
        ...
        self.trigger_metadata = AutoModerationTriggerMetadata._from_data(
            data=data["trigger_metadata"]
        )
        self.actions: list[AutoModerationAction] = [
            AutoModerationAction._from_data(data=d) for d in data["actions"]
        ]
        self.enabled = data["enabled"]
        self.exempt_role_ids = [try_snowflake(d) for d in data["exempt_roles"]]
        self.exempt_channel_ids = [try_snowflake(d) for d in data["exempt_channels"]]
